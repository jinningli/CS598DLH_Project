...
class LSTM(nn.Module):
    def __init__(self, opt):
        ...
        self.linears = nn.Sequential (
            nn.Linear ( opt.hidden_size * 2 if not SKIP else opt.hidden_size * 3, opt.rnn_size ),
            # nn.ReLU ( ),
            # nn.Dropout ( 0.25 ),
    ...
    def forward_2(self, x, master, mask=None, time=None, phase='train', value=None, trend=None):
        ...
        if SKIP:
            skip_out = torch.transpose(x, 1, 2).contiguous()
        ...
        for i in range(start, lstm_out.size(2), delta):
            if SKIP:
                skip_pool_out.append(self.pooling(skip_out[:, :, :i]))
            pool_out.append(self.pooling(lstm_out[:,:, :i]))
            mask_out.append(mask[:, i])
            time_out.append(time[:, i])
        if SKIP:
            skip_pool_out.append(self.pooling(skip_out))
        ...
        if SKIP:
            skip_out = torch.cat(skip_pool_out, 2)
        ...
        if SKIP:
            skip_out = torch.transpose(skip_out, 1, 2).contiguous()
        lstm_out = torch.transpose(lstm_out, 1, 2).contiguous() # (bs, 98, 512)

        out_vital = self.linears(lstm_out if not SKIP else torch.cat([lstm_out, skip_out], dim=2))
    ...
    def forward_1(self, x, master, mask=None, time=None, phase='train', value=None, trend=None):
        ...
        if SKIP:
            skip_out = torch.transpose(x, 1, 2).contiguous()
            skip_out = self.pooling(skip_out)
            skip_out = skip_out.view(skip_out.size(0), -1)
            combined_out = torch.cat([lstm_out, skip_out], dim=1)
        else:
            combined_out = lstm_out
        out = self.linears(combined_out) + self.master_linear(master)
        return out
